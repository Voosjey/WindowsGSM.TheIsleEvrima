        ...
        public async void CreateServerCFG()
        {
            ...
            if (!File.Exists(engineIniPath))
            {
                try
                {
                    ...
                    {
                        await webClient.DownloadFileTaskAsync($"https://raw.githubusercontent.com/Voosjey/The-Isle-Evrima-ini/refs/heads/main/Engine.ini", engineIniPath);
                    }
                }
                catch (Exception e)
                {
...
string win64 = Path.Combine(serverRoot, @"TheIsle\Binaries\Win64\");
...
foreach (string dll in optionalDlls)
{
    string source = Path.Combine(serverRoot, dll);
    string target = Path.Combine(win64, dll);
    ...
    try
    {
        if (File.Exists(source))
        {
            if (!File.Exists(target))
            {
                ...
                Debug.WriteLine($"[TheIsle] Copied optional DLL: {dll}");
            }
        }
        else
        {
            Debug.WriteLine($"[TheIsle] Optional DLL not found (ignored): {dll}");
        }
    }
    catch (Exception ex)
    {
        Debug.WriteLine($"[TheIsle] DLL copy skipped ({dll}): {ex.Message}");
    }
}
        ...
        public static async Task<bool> DownloadGameServerConfig(string fileSource, string filePath, string iniType)
        {
            ...
            string downloadUrl = iniType == "Game" 
                ...
                : $"https://raw.githubusercontent.com/Voosjey/The-Isle-Evrima-ini/refs/heads/main/Engine.ini"; // Add Engine.ini URL here
